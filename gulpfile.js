...
function copy() {
  return src(sourceFolder + '/favicons/*.*').pipe(dest(buildFolder + '/favicons'))
}
...
function serve() {
  ...
  watch(sourceFolder + '/favicons/**/*', series(copy)).on('change', sync.reload)
};


exports.build = series(clear, scss, js, img, sprite, fonts, html, clearBlocksDir);
exports.default = series(clear, scss, js, img, svg, sprite, fonts, html, copy, clearBlocksDir, serve);
